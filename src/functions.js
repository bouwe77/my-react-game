...
export function movePlayer(maze, direction, onMove = null) {
  ...
  // Decrement the food counter if the player ate food and add new food.
  if (maze.items[newIndex].type === itemType.FOOD) {
    updatedMaze.currentFoods--;
    updatedMaze = addFood(updatedMaze);
  }

  // Determine whether the player reached the exit.
  if (updatedMaze.playerIndex === updatedMaze.exitIndex)
    updatedMaze.reachedExit = true;
